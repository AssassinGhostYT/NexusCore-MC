...
use crate::protocol::varint::{write_varu32, write_vari32, read_vari32};
...
pub fn pack_single_block_storage(runtime_id: u32) -> Vec<u8> {
    ...
    // NOTE: palette size is NOT written when bits_per_block == 0
...
pub fn make_flat_chunk_payload() -> Vec<u8> {
    ...
    let air_storage = pack_single_block_storage(Air::RUNTIME_ID);
    
    // Subchunk 0 (Y = -64..-49) contains Bedrock at the base, and the rest is Air
    // Local Y = 0 (Y = -64): Bedrock (infiniburn = false) -> Runtime ID 13079
    // Local Y = 1..=15 (Y = -63..-49): Air -> Runtime ID 12530
    ...
    // Write Subchunk 0
    payload.push(8); // SubChunk Version: 8
    payload.push(1); // Storage layers count: 1
    payload.extend_from_slice(&sub0_storage);
    
    // Subchunks 1..23 (Y = -48..319) are all Air
    for _ in 1..24 {
        payload.push(8); // SubChunk Version: 8
        payload.push(1); // Storage layers count: 1
        payload.extend_from_slice(&air_storage);
    }
